...
async function github<T>(path:string, revalidate=300):Promise<T>{
  try {
    const response=await fetch(`https://api.github.com${path}`,{headers:{Accept:"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28",...(process.env.GITHUB_TOKEN?{Authorization:`Bearer ${process.env.GITHUB_TOKEN}`}:{})},next:{revalidate}});
...
export const searchRepos=(q:string,page=1)=>github<SearchResult<GitHubRepo>>(`/search/repositories?q=${encodeURIComponent(q)}&sort=stars&per_page=12&page=${page}`,60);
