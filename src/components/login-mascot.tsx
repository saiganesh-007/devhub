...
/**
 * DevHub cat — roaming virtual-pet mascot for the login environment.
 *
 * Layered slow gaze (eyes → head → body → tail with delay + inertia),
 * curiosity + petting overlays, tap reactions (surprised / annoyed / dizzy),
 * form-priority state machine, walk/run roaming. All continuous motion is
 * ref-driven (translate3d, no per-frame setState).
 *
 * Priority: loading/error/success > dizzy/recovering > password >
 * email > tap reactions > curiosity/happy > button > walk/run > idle.
 */
export function LoginMascot() {
  ...
  useEffect(() => {
    ...
    function pointerWeight() {
      ...
      if (m === "password" || m === "peek" || m === "loading" || m === "dizzy") return 0;
    ...
    function onFocusOut() {
      ...
      window.setTimeout(() => {
        ...
        if (
          cur === "email" || cur === "recognized" || cur === "password" ||
    ...
    function onMoodEvent(e: Event) {
      ...
      if (detail === "peek") {
        if (moodRef.current !== "password" && moodRef.current !== "peek") return;
        applyMood("peek");
      ...
      if (detail === "button") {
        if (moodRef.current === "idle") applyMood("button");
        return;
      }
      if (detail === "idle") {
        const m = moodRef.current;
        if (m === "idle" || m === "button" || m === "curious" || m === "happy") applyMood("idle");
        return;
      }
      if (detail === "success") {
        clearTransient();
    ...
    function tick() {
      ...
      if (!roam) {
        ...
        tgt.active = false;
      } else if (mood === "email" || mood === "recognized") {
        face(1);
        tgt.x = bounds.x - 4;
        tgt.y = clamp(pos.y * 0.9, -bounds.y, bounds.y);
        tgt.active = true;
        setGaitBoth("walk");
      } else if (mood === "password" || mood === "peek") {
        face(-1);
        tgt.x = -(bounds.x - 6);
    ...
    function scheduleBlink() {
      ...
      blinkTimeout = window.setTimeout(() => {
        ...
        if (!reduceMotion.matches && moodRef.current === "happy") {
    ...
    window.addEventListener("devhub:login-mood", onMoodEvent as EventListener);
    ...
    return () => {
      ...
      window.removeEventListener("devhub:login-mood", onMoodEvent as EventListener);
